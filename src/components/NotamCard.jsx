...
const NotamCard = ({
  ...
  return (
    <div
      ...
      onClick={() => {
        if (needsToExpand) {
          onCardClick(key, notam);
        } else if (isNew) {
          onMarkAsViewed(notam);
        }
      }}
      style={cardStyles}
    >
      <div className={`card-head ${headClass}`} style={{
        padding: '0.75rem',
        fontSize: '1.1rem',
        fontWeight: 'bold',
        display: 'flex',
          ...
          <div style={{display: 'flex', alignItems: 'center'}}>
            {timeStatus && (
              <span style={{
                ...
                marginRight: '0.5rem',
                ...TIME_STATUS_STYLES[timeStatus.toLowerCase()]
              }}>
                {timeStatus}
              </span>
            )}
      ...
      <div className="notam-card-content" style={{
        ...
        {needsToExpand && (
          ...
          >
            <i className={`fa fa-angle-${expanded ? "up" : "down"}`} style={{fontSize: '0.875rem'}}></i>
          </button>
        )}
      </div>
    </div>
